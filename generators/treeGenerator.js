...
export default {
    ...
    coordSystem(vec3) {  //vec3 non-zero Vector3
    ...
    */
    createBranch(branchAt, branchSys, branchLength, branchTaper, branchSlices, bowFreq, bowHeight, branchRadius, scene) {          
    ...
    */
    createTreeBase(trunkHeight, trunkTaper, trunkSlices, boughs, forks, forkAngle, forkRatio, bowFreq, bowHeight, scene) {   
        ...
        var _trunk_sys = this.coordSystem(_trunk_direction);    
        ...
        var _trunk = this.createBranch(_trunk_root_at, _trunk_sys, trunkHeight, trunkTaper, trunkSlices, 1, bowHeight, 1, scene); //create trunk
        ...
        _turn = this.randPct(f*_fork_turn, 0.25);	//randomise angle of spread for a fork	
        _fork_branch_direction = _trunk_sys.y.scale(Math.cos(this.randPct(forkAngle,0.15))).add(_trunk_sys.x.scale(Math.sin(this.randPct(forkAngle,0.15))*Math.sin(_turn))).add(_trunk_sys.z.scale(Math.sin(this.randPct(forkAngle,0.15))*Math.cos(_turn)));
        _fork_branchSys = this.coordSystem(_fork_branch_direction);      		 
        _branch = this.createBranch(_top_point, _fork_branchSys, trunkHeight*forkRatio, trunkTaper, trunkSlices, bowFreq, bowHeight*_PHI, trunkTaper, scene);
        ...
        if(boughs > 1) { // When boughs = 2 create further branches at end of new branch
            for(var k =0; k<forks; k++) {
                _bough_turn = this.randPct(k*_fork_turn, 0.25);
                _bough_direction = _fork_branchSys.y.scale(Math.cos(this.randPct(forkAngle,0.15))).add(_fork_branchSys.x.scale(Math.sin(this.randPct(forkAngle,0.15))*Math.sin(_bough_turn))).add(_fork_branchSys.z.scale(Math.sin(this.randPct(forkAngle,0.15))*Math.cos(_bough_turn)));
                _bough_sys = this.coordSystem(_bough_direction);       		 
                _bough = this.createBranch(_bough_top, _bough_sys, trunkHeight*forkRatio*forkRatio, trunkTaper, trunkSlices, bowFreq, bowHeight*_PHI*_PHI, trunkTaper*trunkTaper, scene);
    ...
    */
    createTree(trunkHeight, trunkTaper, trunkSlices, trunkMaterial, boughs, forks, forkAngle, forkRatio, branches, branchAngle, bowFreq, bowHeight, leavesOnBranch, leafWHRatio, leafMaterial, scene) {        
        if(!(boughs ==1 || boughs ==2)) {
            boughs = 1;
        }  
        //create base tree  
        var _base = this.createTreeBase(trunkHeight, trunkTaper, trunkSlices, boughs, forks, forkAngle, forkRatio, bowFreq, bowHeight, scene);    
        ...
        for(var f=0; f<Math.pow(forks, boughs + 1); f++) {
            _turns.push(this.randPct(Math.floor(f / Math.pow(forks, boughs)) * _fork_turn, 0.2))
        }
        ...
        var _set_mini_trees = function(particle, i, s) {       
            ...
            var turn = _turns[s];             
            var _mini_direction = _mini_sys.y.scale(Math.cos($this.randPct(forkAngle,0))).add(_mini_sys.x.scale(Math.sin($this.randPct(forkAngle,0))*Math.sin(turn))).add(_mini_sys.z.scale(Math.sin($this.randPct(forkAngle,0))*Math.cos(turn)));
        ...
        var _set_branches = function(particle, i, s) {              
            ...
            var _turn = _bturns[s];             
            var _mini_direction = _mini_sys.y.scale(Math.cos($this.randPct(branchAngle,0))).add(_mini_sys.x.scale(Math.sin($this.randPct(branchAngle,0))*Math.sin(_turn))).add(_mini_sys.z.scale(Math.sin($this.randPct(branchAngle,0))*Math.cos(_turn)));
